...
opcode_stub_override_t opcode_overrides_by_stub(opcode_dict_t d, opcode_stub_t stub) {
	return (opcode_stub_override_t) hash_find(&d->overrides_by_stub, (hash_key) stub);
}

void opcode_set_stub_overrides(opcode_dict_t d, opcode_stub_t stub, opcode_stub_override_t ovl) {
	opcode_stub_override_t initial = opcode_overrides_by_stub(d, stub);
	if(!initial) {
		hash_addelem(&d->overrides_by_stub, (hash_key) stub, (hash_elem) ovl);
		ovl->argc = ovl->offset;
	} else {
		while(initial->next) {
