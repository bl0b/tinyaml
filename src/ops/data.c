...
void _VM_CALL vm_op__vcls_gname(vm_t vm, word_t unused) {
	char* str = (char*) dynamic_cast(vm, _vm_pop(vm), DataString, NULL, NULL);
	vobj_class_t cls = (vobj_class_t) dynamic_cast(vm, _vm_pop(vm), DataObjVCls, NULL, NULL);
...
void _VM_CALL vm_op__vcls_gbase(vm_t vm, word_t unused) {
	vobj_class_t cls = (vobj_class_t) dynamic_cast(vm, _vm_pop(vm), DataObjVCls, NULL, NULL);
	vm_push_data(vm, cls->_base.type, cls->_base.data);
}

void _VM_CALL vm_op__vcls_sbase(vm_t vm, word_t unused) {
	vm_data_t d = _vm_pop(vm);
	vobj_class_t cls = (vobj_class_t) dynamic_cast(vm, _vm_pop(vm), DataObjVCls, NULL, NULL);
	int is_array=0;
	dynarray_t da = (dynarray_t) dynamic_cast(vm, d, DataObjArray, NULL, &is_array);
	if(is_array) {
		cls->_base.type = DataObjArray;
		cls->_base.data = (word_t) da;
	} else {
		cls->_base.type = DataObjVCls;
		cls->_base.data = dynamic_cast(vm, d, DataObjVCls, NULL, &is_array);
		if(!is_array) {
			raise_exception(vm, DataString, "Type");
		}
	}
	if(cls->_base.type&DataManagedObjectFlag) {
		vm_obj_ref_ptr(vm, (void*)cls->_base.data);
	}
}
...
void _VM_CALL vm_op__vcls_soo(vm_t vm, word_t unused) {
	...
	vm_printerrf("set override %s:%i => %p in %p\n", oc, ocarg, df, cls);
	vclass_set_override(cls, opcode_stub_by_name(&vm->opcodes, ocarg, oc), &d);
}
