...
void vm_execute_inline(vm_t vm, vm_dyn_func_t df, word_t argc, ...) {
	...
	va_start(va, argc);
	for(;i<argc;i+=1) {
		dt = va_arg(va, vm_data_type_t);
		d = va_arg(va, word_t);
		vm_push_data(vm, dt, d);
	}
	va_end(va);
	vm_push_data(vm, DataInt, argc);
	stub_engine->_run_sync((vm_engine_t)&stub_engine, df->cs, df->ip, vm->current_thread?vm->current_thread->prio:50);
}
...
word_t dynamic_cast(vm_t vm, vm_data_t d, vm_data_type_t newtype, vobj_class_t newcls, int*fail) {
	...
	if(newcls!=NULL) {
		...
		case DataObjVObj:
			...
			df = (vm_dyn_func_t)dynamic_cast(vm, &objcls->_cast_to_class, DataObjFun, NULL, NULL);
			if(df) {
				vm_execute_inline(vm, df, 1, d->type, d->data);
				d=_vm_pop(vm);
				return d->data;
			}
			/* search bases */
...
vm_dyn_func_t find_override(vobj_ref_t obj, opcode_stub_t stub) {
	hashtab_t oo = obj&&obj->cls ? &obj->cls->_overrides : NULL;
	vm_data_t d = oo?hash_find(oo, (hash_key) stub):NULL;
	printf("   find_override obj=%p cls=%p oo=%p stub=%p => dt=%u d=%8.8lX\n", obj, obj?obj->cls:NULL, oo, stub, d?d->type:0, d?d->data:0);
	return d?(vm_dyn_func_t)dynamic_cast(_glob_vm, d, DataObjFun, NULL, NULL):NULL;
}
...
// */
	...
	if(*array&&!setjmp(_glob_vm_jmpbuf)) {
		...
		opcode_stub_override_t ovl = opcode_overrides_by_stub(&vm->opcodes, (opcode_stub_t)*array);
		while(ovl&&!method) {
			if(_vm_trace) { printf("override offset %i\n", ovl->offset); }
			if(gstack_size(&t->data_stack)>=ovl->offset) {
				offset = ovl->offset;
				d = gpeek( vm_data_t , &t->data_stack, -offset);
				obj = (vobj_ref_t) d->data;
				method = d->type==DataObjVObj?find_override(obj, ovl->target):NULL;
				argc=ovl->argc;
				if(_vm_trace) {
